...
def prompt_seed_update(json_data):
    ...
    for k, v in json_data['prompt'].items():
        ...
        cls = v['class_type']
        if cls.lower() == 'global seed (focus nodes)'.lower():
            seed_mode = v['inputs']['seed_mode']
...
def workflow_seed_update(json_data):
    ...
    # Sort nodes so 'Global Seed (Focus Nodes)' is processed first
    nodes.sort(key=lambda x: x['type'] != 'Global Seed (Focus Nodes)')

    for node in nodes:
        node_id = str(node['id'])
        if node_id in prompt:
            if node['type'] == 'Global Seed (Focus Nodes)':
